/*
五子棋函数  wuziqi.org.cn v7.0
*/
var fiveChess_online = {
	NO_CHESS: 0,
	BLACK_CHESS: -1,
	WHITE_CHESS: 1,
	chessArr: [],	//记录棋子
		...
		$(".operating-panel a").click(function (event) {
			...
			switch (id) {
				...
				case "second_move_btn":
					...
					break;
				case "replay_btn":
					if (!_fiveChess.ready) {	//点准备
						_fiveChess.ready = true;
						_fiveChess.weight_user = Math.random();
						msg_send("已准备", _fiveChess.weight_user);
						$("#user1_status").text("已准备");
						$("#replay_btn").html("取消&nbsp;&nbsp;&nbsp;准备");
						if (_fiveChess.ready_against) {
							if (_fiveChess.weight_user > _fiveChess.weight_againist) {
								_fiveChess.isPlayerTurn = true;
								_fiveChess.humanPlayer = "black";
								_fiveChess.AIPlayer = "white";
								$("#first_move_btn").addClass("selected");
								$("#second_move_btn").removeClass("selected");
								$("#black_btn").addClass("selected");
								$("#white_btn").removeClass("selected");
							}
							else {	
								_fiveChess.isPlayerTurn = false;
								_fiveChess.humanPlayer = "white";
								_fiveChess.AIPlayer = "black";
								$("#first_move_btn").removeClass("selected");
								$("#second_move_btn").addClass("selected");
								$("#black_btn").removeClass("selected");
								$("#white_btn").addClass("selected");
							}
						...
						{
							$("#user1_status").text("未准备");
							_fiveChess.ready = false;
							msg_send("取消准备", "");
							$("#replay_btn").html("准&nbsp;&nbsp;&nbsp;备");
						}
						//_fiveChess.gameStart();
	...
	showResult: function (isPlayerWin) {
		this.ready=false;
		this.ready_against=false;
		$("#user2_status").text("未准备");
		$("#user1_status").text("未准备");
		$("#replay_btn").html("准&nbsp;&nbsp;&nbsp;备");
		if (isPlayerWin) {
